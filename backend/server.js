...
app.get('/api/afip/padron/:doc', async (req, res) => {
  ...
  try {
    ...
    // 2. Si no está en base local, consultar a Web Services de AFIP / ARCA (ws_sr_padron_a13 / ws_sr_constancia_inscripcion)
    let afipClientData = null;
    try {
      let businessConfig = DEFAULT_BUSINESS_CONFIG;
      if (dbConnected) {
        const bRes = await query("SELECT value FROM app_settings WHERE key = 'business_config'");
        if (bRes.rowCount > 0 && bRes.rows[0].value) {
          businessConfig = { ...DEFAULT_BUSINESS_CONFIG, ...bRes.rows[0].value };
        }
      }

      const afipConfig = businessConfig.afip || {};
      const originCuit = String(afipConfig.cuit || afipConfig.claveFiscalCuit || '20384920194').replace(/[^0-9]/g, '');

      // Determinar posibles identificadores fiscales (CUIT directo o CUILs derivados de DNI)
      const isCuit = cleanDoc.length === 11;
      const isDni = cleanDoc.length >= 7 && cleanDoc.length <= 8;
      const targetCuits = [];

      if (isCuit) {
        targetCuits.push(cleanDoc);
      } else if (isDni) {
        const cuilM = calculateCuilMod11(cleanDoc, 'M')?.replace(/[^0-9]/g, '');
        const cuilF = calculateCuilMod11(cleanDoc, 'F')?.replace(/[^0-9]/g, '');
        if (cuilM) targetCuits.push(cuilM);
        if (cuilF && cuilF !== cuilM) targetCuits.push(cuilF);
      }

      // Intentar consultar con Afip SDK si existe
      if (targetCuits.length > 0) {
        try {
          const AfipModule = await import('@afipsdk/afip.js');
          const Afip = AfipModule.default || AfipModule;
          const afipInstance = new Afip({
            CUIT: parseInt(originCuit || '20384920194', 10),
            production: afipConfig.environment === 'produccion'
          });

          for (const targetCuit of targetCuits) {
            // A. Probar con RegisterScopeThirteen (Padrón A13 - Oficial de AFIP)
            if (afipInstance.RegisterScopeThirteen) {
              try {
                const persona = await afipInstance.RegisterScopeThirteen.getTaxpayerDetails(targetCuit);
                if (persona) {
                  const nom = (persona.razonSocial || `${persona.apellido || ''} ${persona.nombre || ''}`).trim();
                  const dom = persona.domicilioFiscal || {};
                  const dirParts = [dom.direccion, dom.localidad, dom.descripcionProvincia].filter(Boolean);
                  const dirStr = dirParts.join(', ');

                  let taxCond = 'Consumidor Final';
                  const cat = String(persona.categoria || '').toUpperCase();
                  if (cat.includes('RESPONSABLE') || cat.includes('INSCRIPTO')) taxCond = 'Responsable Inscripto';
                  else if (cat.includes('MONOTRIBUTO')) taxCond = 'Monotributista';
                  else if (cat.includes('EXENTO')) taxCond = 'Exento';

                  afipClientData = {
                    name: nom.toUpperCase() || 'CONTRIBUYENTE REGISTRADO',
                    phone: '',
                    email: '',
                    address: dirStr || '',
                    docType: isCuit ? 'CUIT' : 'DNI',
                    docNumber: cleanDoc,
                    cuit: `${targetCuit.slice(0, 2)}-${targetCuit.slice(2, 10)}-${targetCuit.slice(10)}`,
                    taxCondition: taxCond,
                    suggestedInvoiceType: taxCond === 'Responsable Inscripto' ? 'FACTURA_A' : 'FACTURA_B',
                    source: 'afip_oficial'
                  };
                  break;
                }
              } catch (e13) {
                // Silencioso, pasa al siguiente intento
              }
            }

            // B. Probar con RegisterInscriptionProof (Constancia de inscripción)
            if (!afipClientData && afipInstance.RegisterInscriptionProof) {
              try {
                const details = await afipInstance.RegisterInscriptionProof.getTaxpayerDetails(targetCuit);
                if (details) {
                  const nom = (details.denominacion || details.razonSocial || `${details.apellido || ''} ${details.nombre || ''}`).trim();
                  const dom = details.domicilioFiscal || {};
                  const dirParts = [dom.direccion, dom.localidad, dom.descripcionProvincia].filter(Boolean);
                  const dirStr = dirParts.join(', ');
                  ...
                  afipClientData = {
                    ...
                    docNumber: cleanDoc,
                    cuit: `${targetCuit.slice(0, 2)}-${targetCuit.slice(2, 10)}-${targetCuit.slice(10)}`,
                    taxCondition: details.categoriaIva || 'Consumidor Final',
                    suggestedInvoiceType: String(details.categoriaIva || '').toUpperCase().includes('INSCRIPTO') ? 'FACTURA_A' : 'FACTURA_B',
                    source: 'afip_constancia'
                  };
                  break;
                }
              } catch (eProof) {
          ...
          // Afip SDK no disponible o sin conexión
        }
      }
    } catch (eAfip) {
      console.warn('⚠️ Consulta externa a AFIP no disponible:', eAfip.message);
    }

    // 3. Si AFIP devolvió datos del contribuyente, guardarlo en PostgreSQL y responder
    if (afipClientData) {
      if (dbConnected) {
        await query(
          `INSERT INTO app_settings (key, value, updated_at)
          ...
          [`client_${cleanDoc}`, JSON.stringify(afipClientData)]
        ).catch(() => null);
      }
      ...
      return res.json({
        success: true,
        found: true,
        source: afipClientData.source || 'afip_padron',
        client: afipClientData
      });
    }

    // 4. Si no se encontró en AFIP ni en la base local (ciudadano particular / consumidor final puro)
    const isCuit = cleanDoc.length === 11;
    const isDni = cleanDoc.length >= 7 && cleanDoc.length <= 8;
