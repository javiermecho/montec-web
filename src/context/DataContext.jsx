...
/**
 * Helper para sustituir de manera segura etiquetas dinámicas {variable} en plantillas
 */
export function formatWhatsAppTemplate(template, vars = {}) {
  if (!template) return '';
    ...
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          whatsappTemplates: { ...DEFAULT_BUSINESS_CONFIG.whatsappTemplates, ...(parsed.whatsappTemplates || {}) }
        };
      }
  ...
  const updateBusinessConfig = (newConfig) => {
    setBusinessConfig(prev => {
      ...
      const merged = {
        ...
        whatsappTemplates: {
          ...(base.whatsappTemplates || DEFAULT_BUSINESS_CONFIG.whatsappTemplates),
          ...(newConfig?.whatsappTemplates || {})
        }
      };
      try {
        localStorage.setItem(STORAGE_KEYS.BUSINESS_CONFIG, JSON.stringify(merged));
